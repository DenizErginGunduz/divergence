...
| G4 | Payoff card (V0) build | OPEN — requirements defined (D-109); discovery and data-rights answers outstanding; the buyer's comparison (D-114) returned "one venue is cheaper wherever either is" — Kalshi, single buckets only (D-116); on Polymarket's dailies the venues are, with one fee-dependent exception, not separable (D-118) |

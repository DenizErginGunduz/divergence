...
python scripts/measure_payoff.py        # the buyer's comparison: which venue sells the same payoff for less (D-114)
...
| `measure_payoff.py` | For the same terminal payoff, is it cheaper to buy on Kalshi or from the option chain, one side's costs only — and does the answer depend on the condition? Verdict rules written before the script (D-114); a zero-fee sensitivity for Deribit's combo rule rides along (D-115). | Kalshi + Deribit |
